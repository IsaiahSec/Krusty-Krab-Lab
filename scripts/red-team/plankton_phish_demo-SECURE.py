...
send(
    label     = "Spoofed Larry -> Krabs (financial fear)",
    from_addr = "larry.lobster@krustykrab.local",
    to_addr   = "eugene.krabs@krustykrab.local",
    subject   = "SECURITY ALERT: Unauthorized Access Attempt on Payroll System",
    body      = f"""Mr. Krabs,

Our monitoring system detected an unauthorized login attempt on the payroll system at 3:47am. Payroll data may have been exposed.

Log into the secure verification portal immediately to lock down the account:

http://10.8.0.5/login

Time is critical. Please do not delay.

- Larry Lobster, Security"""
)

send(
    label     = "Spoofed Larry -> Sandy (IT credential harvest)",
    from_addr = "larry.lobster@krustykrab.local",
    to_addr   = "sandy.cheeks@krustykrab.local",
    subject   = "SECURITY ALERT: Unauthorized Access Attempt on Payroll System",
    body      = f"""Sandy,

Our monitoring system detected an unauthorized login attempt on the mail server admin panel. I need you to verify your credentials immediately:
...
send(
    label     = "Display name spoof - 'Larry Security' from external",
    from_addr = '"Larry Lobster - Security" <security@kk-security.net>',
    to_addr   = "eugene.krabs@krustykrab.local",
    subject   = "CRITICAL: Formula Vault Access Log Anomaly",
    body      = f"""Mr. Krabs,

The formula vault access log shows 3 failed attempts at 2am last night from an unknown workstation. I need you to verify your PIN immediately:

http://10.8.0.5/login

- Larry Lobster, Security Officer"""
)
